...
- REGRA-TOKEN-STORE: token em `sessionStorage` (não `localStorage`, não cookie). Header `Authorization: Bearer <uuid>`. Some ao fechar a aba. Cookie `HttpOnly` fica para quando GAP-EMAIL-REAL + curso pago.
- REGRA-GURU-FRONT: um domínio, um front, vários gurus. Lançamento: guru `java`. Aluno global (mesmo user em qualquer guru).

## NÃO
- NÃO-LOGIN-BKP: não religar o login do bkp (senha + token em localStorage). UI nova = REGRA-CONTA-*.
...
**sessionStorage:** token (UUID cru ou `Bearer <uuid>` — prefixar no header). **Não** senha. **Não** o código depois de validar.
...
- CONTRATO-CONTA-OK `POST /firewall/emailVerification/v1/validateEmail` `{email, readableNumber}` — guarda token em `sessionStorage`, prefixa `Bearer` no header.
...
- GAP-EMAIL-REAL: SES; não pré-preencher código; recaptcha. Aí avaliar cookie `HttpOnly` no lugar de `sessionStorage`.
