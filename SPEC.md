...
- TEST-GLASS-PREVIEW **revogado** (2026-09-03): preview Atual/Sheen/Rim/Sheen+Rim removido (`css/glass-preview.css`, `js/glass-preview.js`, tags GLASS-PREVIEW-TEST, UI Vidro, `localStorage.glassPreview`). **Produto:** Rim ficou permanente em `body.lesson-page .glass` (`--glass-rim-inset` em layout.css; faces cinema em video.css). Não Sheen / Sheen+Rim.
- REGRA-VIDEO-LAYOUT: player padrão = `html/video.html` cinema (gif grande, gavetas de vidro sobre Trianglify, class `layout-cinema`). Controles no palco: tela cheia (Fullscreen API) e modo cinema (esconde os dois menus; o quadro do gif não cresce). **Revogado** (2026-08-28): `html/video3.html` / `html/video4.html` (propostas experimentais). Fundo Trianglify + `.glass` com rim inset permanente (produto). Não são bkp.
